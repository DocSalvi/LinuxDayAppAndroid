...
public class BMDevTag extends BaseLMTag {
	  ...
	  public BMDevTag (GeoTag next, String[] titles, String record, Resources res) throws Exception {
	  ...
	  protected void init (Resources res) {
		  if (icons[0] == null) {
			  try {
				  icons[0] = new PositionIcon(new Dimension(16,19), new Point (8,19), BitmapFactory.decodeResource(res,R.drawable.bmdev_logo));
			  } catch (Exception e) {
				  e.printStackTrace();
			  }
		  }
		  if (icons[1] == null) {
			  try {
				  icons[1] = new PositionIcon(new Dimension(30,36), new Point (14,36), BitmapFactory.decodeResource(res,R.drawable.bmdev_logo_wide));
			  } catch (Exception e) {
				  e.printStackTrace();
			  }
		  }
		  if (description == null) {
			  try {
				  description = new TagDescription(res.getString(R.string.BMDevDescription), icons[0]);
			  } catch (Exception e) {
				  e.printStackTrace();
			  }
		  }
