...
public class BMDevTag extends BaseLMTag {
	  ...
	  protected void init (Resources res, String[] fields) {
		  if (icons[0] == null) {
			  try {
				  icons[0] = new PositionIcon(0.5, 1.0, BitmapFactory.decodeResource(res,R.drawable.bmdev_logo));
			  } catch (Exception e) {
				  e.printStackTrace();
			  }
		  }
		  if (icons[1] == null) {
			  try {
				  icons[1] = new PositionIcon(0.5, 1.0, BitmapFactory.decodeResource(res,R.drawable.bmdev_logo_wide));
			  } catch (Exception e) {
				  e.printStackTrace();
			  }
		  }
		  if (description == null) {
			  try {
				  description = new TagDescription(res.getString(R.string.BMDevDescription), icons[0], "BMDevTag");
			  } catch (Exception e) {
				  e.printStackTrace();
			  }
		  }
		  titleId = R.string.BMDevTitle;
	  }
