...
public class BMEduTag extends BaseLMTag {
	  ...
	  protected void init (Resources res, String[] fields) {
		  if (icons[0] == null) {
			  try {
				  icons[0] = new PositionIcon(0.5, 1.0, BitmapFactory.decodeResource(res,R.drawable.bmedu_logo));
			  } catch (Exception e) {
				  e.printStackTrace();
			  }
		  }
		  if (icons[1] == null) {
			  try {
				  icons[1] = new PositionIcon(0.5, 1.0, BitmapFactory.decodeResource(res,R.drawable.bmedu_logo_wide));
			  } catch (Exception e) {
				  e.printStackTrace();
			  }
		  }
